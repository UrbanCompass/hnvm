...
HNVM reads its configuration from one of three places, in order from lowest to highest specificity:
1. The `.hnvmrc` file that ships with the current version of HNVM
2. An `.hnvmrc` file in your home directory (`~/.hnvmrc`)
3. An `.hnvmrc` file at the root of your git repo
4. An `.hnvmrc` file in the current process working directory (`$PWD/.hnvmrc`)
5. The `package.json` file in the `PWD`
